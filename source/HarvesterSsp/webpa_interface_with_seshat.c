...
void get_parodus_url(char **url)
{
    ...
    if( NULL == seshat_url ) {
        CcspHarvesterConsoleTrace(("RDK_LOG_ERROR, seshat_url is not present in device.properties file\n"));
        return;
    }

    CcspHarvesterConsoleTrace(("RDK_LOG_INFO, seshat_url formed is %s\n", seshat_url));
    if( 0 == init_lib_seshat(seshat_url) ) {
        CcspHarvesterConsoleTrace(("RDK_LOG_INFO, seshatlib initialized! (url %s)\n", seshat_url));
        ...
        if( NULL != discovered_url ) {
            ...
            *url = strndup(discovered_url, discovered_url_size);
            CcspHarvesterConsoleTrace(("RDK_LOG_INFO, seshatlib discovered url = %s, parodus url = %s\n", discovered_url, *url));
            free(discovered_url);
        } else {
            CcspHarvesterConsoleTrace(("RDK_LOG_ERROR, seshatlib registration error (url %s)!", discovered_url));
        }
    } else {
        CcspHarvesterConsoleTrace(("RDK_LOG_ERROR, seshatlib not initialized! (url %s)\n", seshat_url));
    }

    if( NULL == *url ) {
        CcspHarvesterConsoleTrace(("RDK_LOG_ERROR, parodus url (url %s) is not present in seshatlib (url %s)\n", *url, seshat_url));
    }
    ...
    if( NULL != fp ) {
        ...
        while( fscanf(fp,"%s", str) != EOF ) {
            ...
            if( value = strstr(str, "SESHAT_URL=") ) {
                ...
                *url = strdup(value);
                CcspHarvesterConsoleTrace(("RDK_LOG_INFO, seshat_url is %s\n", *url));
            }
        }
    } else {
